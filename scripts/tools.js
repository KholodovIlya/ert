...
class Toggle extends Button {
  constructor(x, y, width, height) { super(x, y, width, height); this.drag = false; this.drag_x = 0; this.value = 0; }

  update() { if(this.drag) { this.change(); this.collide = true; } super.update(); }

  change() {
    this.drag_x = mouse.transform.position.x - this.transform.position.x + this.transform.size.x / 2;
    this.drag_x = this.drag_x < 0 ? 0 : this.drag_x; this.drag_x = this.drag_x > this.transform.size.x ? this.transform.size.x : this.drag_x;
    this.drag_x = float2int(this.drag_x); this.value = this.drag_x / this.transform.size.x;
  }

  onRelease() { this.drag = false; }
  onPress() { this.drag = true; }
}
// UI
